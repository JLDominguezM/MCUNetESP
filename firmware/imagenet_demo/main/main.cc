...
extern "C" const unsigned char g_mcunet_in2[];
extern "C" const unsigned int g_mcunet_in2_len;
...
const char* TAG = "imagenet";
}  // namespace

static void SetupModel() {
    if (g_mcunet_in2_len < 100) {
        ESP_LOGE(TAG, "model_data.cc has placeholder — bake the real .tflite first");
        vTaskDelay(portMAX_DELAY);
    }
    model = tflite::GetModel(g_mcunet_in2);
...
extern "C" void app_main() {
    ESP_LOGI(TAG, "MCUNet-IN2 ImageNet demo");
