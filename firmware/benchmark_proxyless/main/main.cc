...
extern "C" const unsigned char g_proxyless_w0_3[];
extern "C" const unsigned int g_proxyless_w0_3_len;
extern "C" const int8_t g_test_image_in[];
extern "C" const int g_test_image_in_w;
extern "C" const int g_test_image_in_h;

namespace {
constexpr int kInputW = 176;
constexpr int kInputH = 176;
...
const char* TAG = "proxyless";
}  // namespace

static void SetupModel() {
    if (g_proxyless_w0_3_len < 100) {
        ESP_LOGE(TAG, "model_data.cc has placeholder — bake the real .tflite first");
        vTaskDelay(portMAX_DELAY);
    }
    model = tflite::GetModel(g_proxyless_w0_3);
...
extern "C" void app_main() {
    ESP_LOGI(TAG, "ProxylessNAS-w0.3 ImageNet demo");
